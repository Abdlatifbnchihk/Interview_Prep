    ...
    <div class="min-h-screen flex flex-col">
        <nav class="flex items-center justify-between p-6">
            ...
            <div class="flex items-center gap-4">
                @auth
                    <a href="{{ route('domains.index') }}" class="px-5 py-2 text-white/70 hover:text-white transition">
                        Domains
                    </a>
                    <a href="{{ route('concepts.index') }}" class="px-5 py-2 text-white/70 hover:text-white transition">
                        Concepts
                    </a>
                ...
                @else
                    <a href="{{ route('login') }}" class="px-5 py-2 text-white/70 hover:text-white transition">Log in</a>
                    <a href="{{ route('register') }}" class="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg font-medium transition">
                        Register
                    </a>
                @endauth
            </div>
                ...
                <div class="max-w-7xl mx-auto">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
                            <div class="text-2xl font-bold text-white mb-1">{{ $domains->count() }}</div>
                            <div class="text-white/40 text-sm">Total Domains</div>
                        </div>
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
                            <div class="text-2xl font-bold text-white mb-1">{{ $totalConcepts }}</div>
                            <div class="text-white/40 text-sm">Total Concepts</div>
                        </div>
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-5">
                            <div class="text-2xl font-bold text-emerald-400 mb-1">{{ $totalMastered }}</div>
                    ...
                    <div class="flex items-center justify-between mb-5">
                        ...
                        <a href="{{ route('domains.create') }}" class="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium transition text-sm shadow-lg shadow-indigo-500/20">
                    ...
                    @if ($domains->isEmpty())
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-16 text-center">
                            ...
                            <a href="{{ route('domains.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-medium transition">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            @foreach ($domains as $domain)
                                <div class="group bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl overflow-hidden hover:bg-white/10 hover:border-white/20 transition-all">
                                    <div class="h-1.5" style="background-color: {{ $domain->color }};"></div>
                                    <div class="p-5">
                                        <div class="flex items-start justify-between mb-4">
                                            <div class="flex items-center gap-3">
                                                <div class="w-3 h-3 rounded-full shadow-sm" style="background-color: {{ $domain->color }};"></div>
                                                <h3 class="text-base font-semibold text-white">{{ $domain->name }}</h3>
                                            </div>
                                            <div class="flex gap-1 opacity-0 group-hover:opacity-100 transition">
                                                <a href="{{ route('domains.edit', $domain) }}" class="p-2 text-white/40 hover:text-indigo-400 transition rounded-lg hover:bg-white/5">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <a href="{{ route('domains.show', $domain) }}" class="flex items-center justify-center gap-2 w-full py-2.5 bg-white/5 hover:bg-white/10 text-white/60 hover:text-white rounded-xl transition text-sm font-medium">
                                            View Domain
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @else
                ...
                <div class="max-w-6xl mx-auto">
                    ...
                    <div class="text-center mt-12">
                        ...
                        <a href="{{ route('register') }}" class="inline-block px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-xl font-semibold transition transform hover:scale-105">
                            Get Started Free
                        </a>
                    </div>
                </div>
