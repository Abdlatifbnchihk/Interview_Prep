    ...
    <div class="pt-20">
        <div class="pt-4 pb-8">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ route('concepts.index') }}" class="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 hover:text-white rounded-xl font-medium transition text-sm">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-3 mb-1 flex-wrap">
                                    ...
                                    @if ($concept->domain)
                                        <span class="px-2 py-0.5 text-xs rounded-full bg-white/5 text-white/40 flex-shrink-0 flex items-center gap-1">
                                            <span class="w-1.5 h-1.5 rounded-full" style="background-color: {{ $concept->domain->color }};"></span>
                                            {{ $concept->domain->name }}
                                        </span>
                                    @else
                                ...
                                <div class="flex items-center gap-3 text-xs text-white/30">
                                    <span class="flex items-center gap-1.5">
                                        ...
                                        Deleted {{ $concept->deleted_at->diffForHumans() }}
                                    </span>
                                    <span class="px-2 py-0.5 text-xs rounded-full bg-{{ $concept->difficulty->color() }}-500/20 text-{{ $concept->difficulty->color() }}-400">
                                        {{ $concept->difficulty->label() }}
                                    </span>
                                </div>
                            </div>
                            <div class="flex items-center gap-2 flex-shrink-0">
                                <form action="{{ route('concepts.restore', $concept->id) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                                ...
                                <form action="{{ route('concepts.forceDelete', $concept->id) }}" method="POST" onsubmit="return confirm('Permanently delete this concept and ALL its questions? This cannot be undone.');">
                                    @csrf
                                    @method('DELETE')
