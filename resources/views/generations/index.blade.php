    ...
    <div class="pt-20">
        <div class="pt-4 pb-8">
            <div class="flex items-center justify-between mb-6">
                <a href="{{ route('concepts.show', $concept) }}" class="inline-flex items-center gap-2 text-white/50 hover:text-white transition text-sm mb-4">
                        ...
                        <div class="bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-5">
                            <div class="flex items-center justify-between mb-4">
                                <div class="flex items-center gap-2 text-white/40 text-xs">
                                    ...
                                    {{ $generation->created_at->format('M d, Y H:i') }}
                                </div>
                                <form action="{{ route('generations.destroy', $generation) }}" method="POST" onsubmit="return confirm('Delete this generation?');">
                                    @csrf
                                    @method('DELETE')
                            ...
                            <ul class="space-y-2.5">
                                @foreach ($generation->questions as $index => $question)
                                    <li class="flex items-start gap-3">
                                        ...
                                        <span class="text-white/70 text-sm leading-relaxed">{{ $question->question }}</span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
